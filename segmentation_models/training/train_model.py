...
from segmentation_models.dataset.load_dataset import create_dataset
from segmentation_models.models.fcn_model import fcn_model
from segmentation_models.models.unet_model import unet_model


def train_net(num_epochs=10, batch_size=64, val_subsplits=5, net_model='unet', net_name='unet_dice'):
    ...
    with tf.device('gpu'):
        if net_model == 'unet':
            ...
            net_model = fcn_model()
        net_model.compile(optimizer=Adam(learning_rate=0.0001), loss=[dice_coef_loss], metrics=['accuracy'])
